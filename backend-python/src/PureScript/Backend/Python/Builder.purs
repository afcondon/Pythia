...
generateRuntime outputDir = do
  ...
  runtimeCode = String.joinWith "\n"
    ...
    , "    'shiftImpl', 'unshiftAllImpl', 'spliceImpl', 'toAssocArrayImpl', 'lengthSTImpl',"
    ...
    , "    return step2"
    , ""
    , "def spliceImpl(start):"
    , "    def step2(deleteCount):"
    , "        def step3(items):"
    , "            def step4(xs):"
    , "                def st():"
    ...
    , "    def st():"
    , "        return [{'index': i, 'value': x} for i, x in enumerate(xs)]"
    , "    return st"
    , ""
    , "# lengthImpl for ST arrays (same as regular length but returns ST)"
    , "def lengthSTImpl(xs):"
    , "    def st():"
    , "        return len(xs)"
    , "    return st"
    ...
    , "    return effect"
    , ""
    , "# Aliases without underscores for Effect.Ref"
    , "new = _new"
    , "read = _read"
    , "write = _write"
    , "modify = _modify"
    ...
    , ""
    , "runSTFn1 = lambda f: lambda a: f(a)"
    , "runSTFn2 = lambda f: lambda a: lambda b: f(a, b)"
    , "runSTFn3 = lambda f: lambda a: lambda b: lambda c: f(a, b, c)"
    , "runSTFn4 = lambda f: lambda a: lambda b: lambda c: lambda d: f(a, b, c, d)"
    , "runSTFn5 = lambda f: lambda a: lambda b: lambda c: lambda d: lambda e: f(a, b, c, d, e)"
    , "runSTFn6 = lambda f: lambda a: lambda b: lambda c: lambda d: lambda e: lambda g: f(a, b, c, d, e, g)"
    , "runSTFn7 = lambda f: lambda a: lambda b: lambda c: lambda d: lambda e: lambda g: lambda h: f(a, b, c, d, e, g, h)"
    , "runSTFn8 = lambda f: lambda a: lambda b: lambda c: lambda d: lambda e: lambda g: lambda h: lambda i: f(a, b, c, d, e, g, h, i)"
    , "runSTFn9 = lambda f: lambda a: lambda b: lambda c: lambda d: lambda e: lambda g: lambda h: lambda i: lambda j: f(a, b, c, d, e, g, h, i, j)"
    , "runSTFn10 = lambda f: lambda a: lambda b: lambda c: lambda d: lambda e: lambda g: lambda h: lambda i: lambda j: lambda k: f(a, b, c, d, e, g, h, i, j, k)"
    , ""
