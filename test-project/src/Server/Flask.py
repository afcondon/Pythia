...
def _to_json_safe(val):
    """Recursively convert PureScript data structures to JSON-safe Python."""
    if isinstance(val, dict):
        return {k: _to_json_safe(v) for k, v in val.items()}
    elif isinstance(val, (list, tuple)):
        # Check if it's a tagged tuple (ADT)
        if len(val) > 0 and isinstance(val[0], str) and val[0][0].isupper():
