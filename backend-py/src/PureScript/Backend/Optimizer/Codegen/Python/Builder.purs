-- | Generic build driver for a backend-optimizer consumer.
-- |
-- | This is a verbatim port of backend-es's
-- | @PureScript.Backend.Optimizer.Codegen.EcmaScript.Builder@ -- it contains
-- | nothing ES-specific (it only references the optimizer's own modules), but
-- | it lives inside the @backend-es@ package, so we reproduce it here under our
-- | own namespace rather than depend on @backend-es@. It reads/sorts corefn.json
-- | modules, merges default + external inline directives, and folds the
-- | optimizer's @buildModules@ over them, handing each optimized 'BackendModule'
-- | to our codegen via 'onCodegenModule'.
module PureScript.Backend.Optimizer.Codegen.Python.Builder where
